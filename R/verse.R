...
BIGverse_attach <- function() {
  ...
  msg(
    cli::rule(
      ...
      right = paste0("BIGverse ", package_version("BIGverse"))
    ),
    startup = TRUE
  )
...
#' Conflicts between the BIGverse and other packages
#'
#' This function lists all the conflicts between packages in the BIGverse
...
#' @examples
#' BIGverse_conflicts()
BIGverse_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  pkg_names <- paste0("package:", BIGverse_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% pkg_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "BIGverse_conflicts")
}

BIGverse_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "BIGverse_conflicts()"
  )
...
#' @export
print.BIGverse_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(BIGverse_conflict_message(x))
}
...
#' Update BIGverse packages
#'
#' This will check to see if all BIGverse packages (and optionally, their
...
#' BIGverse_update()
#' }
BIGverse_update <- function(recursive = FALSE) {

  deps <- BIGverse_deps(recursive)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All BIGverse packages up-to-date")
    return(invisible())
  }
...
#' List all BIGverse dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   BIGverse packages.
#' @export
BIGverse_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("BIGverse", pkgs, recursive = recursive)
...
#' List all packages in the BIGverse
#'
#' @param include_self Include BIGverse in the list?
#' @export
#' @examples
#' BIGverse_packages()
BIGverse_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("BIGverse")$Imports
  imports <- strsplit(raw, ",")[[1]]
  parsed <- gsub("^\\s+|\\s+$", "", imports)
  names <- vapply(strsplit(parsed, "\\s+"), "[[", 1, FUN.VALUE = character(1))

  if (include_self) {
    names <- c(names, "BIGverse")
  }
...
.onAttach <- function(...) {
  ...
  if (!"package:conflicted" %in% search()) {
    x <- BIGverse_conflicts()
    msg(BIGverse_conflict_message(x), startup = TRUE)
  }
